...
/**
 *  \file		htdocs/custom/clienjoyholidays/tpl/linkedobjectblock.tpl.php
 *  \ingroup	clienjoyholidays
 *  \brief		Template to show objects linked to clienjoyholidays
 */
...
print "<!-- BEGIN PHP TEMPLATE clienjoyholidays/clienjoyholidays/tpl/linkedobjectblock.tpl.php -->\n";
...
foreach ($linkedObjectBlock as $key => $objectlink) {
	...
	$trclass = 'oddeven';
	$clienjoyholidays_static = new CliEnjoyHolidays($db);
	$trclass = 'oddeven';
	if ($ilink == count($linkedObjectBlock) && empty($noMoreLinkedObjectBlockAfter) && count($linkedObjectBlock) <= 1) {
		$trclass .= ' liste_sub_total';
	}
	echo '<tr class="'.$trclass.'" >';
	echo '<td class="linkedcol-element tdoverflowmax100">'.$langs->trans("CEHTypeObject");
	...
	$result = $clienjoyholidays_static->fetch($objectlink->fk_product);
	if ($result < 0) {
		setEventMessage($clienjoyholidays_static->error, 'errors');
	} elseif ($result > 0) {
		$clienjoyholidays_static->getNomUrl(1);
	}
	print '</td>';
