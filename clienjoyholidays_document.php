...
/**
 *  \file       clienjoyholidays_document.php
 *  \ingroup    clienjoyholidays
 *  \brief      Tab for documents linked to CliEnjoyHolidays
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->clienjoyholidays->multidir_output[$object->entity ? $object->entity : $conf->entity]."/clienjoyholidays/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('clienjoyholidays', 'clienjoyholidays', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("CliEnjoyHolidays")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("CliEnjoyHolidays"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='clienjoyholidays/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'clienjoyholidays/'.dol_sanitizeFileName($object->ref).'/';
