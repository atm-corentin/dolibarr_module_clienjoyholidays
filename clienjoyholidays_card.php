...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('clienjoyholidays', 'clienjoyholidays', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->hasRight('clienjoyholidays', 'clienjoyholidays', 'delete') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->hasRight('clienjoyholidays', 'clienjoyholidays', 'write'); // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->hasRight('clienjoyholidays', 'clienjoyholidays', 'write'); // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/clienjoyholidays/clienjoyholidays_card.php', 1) . '?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($action == 'delete' || ($conf->use_javascript_ajax && empty($conf->dol_use_jmobile))) {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteCliEnjoyHolidays'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 'action-delete');
	}
	...
	if ($action == 'xxx') {
		$text = $langs->trans('ConfirmActionCliEnjoyHolidays', $object->ref);
	...
	*/
	...
	if ($action != 'presend') {
		...
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('clienjoyholidays'));
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/clienjoyholidays/clienjoyholidays_agenda.php', 1) . '?id=' . $object->id);
